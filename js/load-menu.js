...
(function() {
    ...
    // ------------------------------------------------------------------
    // whenPagePresented(cb) - run a callback once the page is really on
    // screen, not merely parsed.
    //
    // Arriving from another page of the site, a crossfade covers the new
    // document for 0.3s (the View Transition in common.css, or the fade
    // above where transitions do not run). Text effects started at
    // DOMContentLoaded therefore played, and finished, underneath it:
    // measured on About -> Works, the crossfade ended at 468ms while the
    // heading stopped scrambling at 440ms, so the visitor saw a static
    // heading. The first navigation of a session escaped this because it
    // uses the fade above rather than a native transition - hence "it only
    // animates the first time, and after a reload".
    // The crossfade was cut from 0.3s to 0.15s (common.css) so this wait
    // stays short; the two together are what keep the effects both quick
    // and visible.
    //
    // The wait is anchored to the first animation frame - this document's
    // first render opportunity, which is when the crossfade over it starts
    // - so a slow load does not stack extra delay. A direct load or reload
    // has no crossfade and starts at once.
    // ------------------------------------------------------------------
    var CROSSFADE_MS = 170; // 0.15s crossfade plus a frame of margin
    ...
    var presented = new Promise(function (resolve) {
        // Safety net: a background tab never gets an animation frame, and
        // the effects must not be stranded if it is opened later.
        var fallback = setTimeout(resolve, 2000);
    ...
    window.whenPagePresented = function (callback) {
        presented.then(callback);
    };
